...
const { daysOfTwoMonth } = useCalendar()
...
<template>
  <div class="px-10 pt-5">
    <div class="relative grid grid-cols-1 gap-x-14 md:grid-cols-2">
      ...
      <section v-for="(month, monthIdx) in daysOfTwoMonth" :key="month.id" class="text-center" :class="[monthIdx === months.length - 1 && 'hidden md:block']">
        <h2 class="text-sm font-semibold text-gray-900">
          {{ month.long }}
        </h2>
        <WeekGrid :days="month.days" />
      </section>
    </div>
    <section class="mt-12">
